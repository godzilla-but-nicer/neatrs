...
impl Graph {
    ...
    pub fn get_layer_map(&self) -> Result<HashMap<usize, usize>, ConnectivityError> {
        ...
        'bfs: while queue.len() > 0 {
            ...
            // we do this before the normal breadth-first searching because we might have to move
            // the current node or that a node in the queue shouldn't properly be in this queue
            // and should instead be in the next
            for node in &queue {
            ...
            // now we know our current node should be in the current layer. we can add it to the map
            rev_layers.insert(*curr, layer);

            // now that we've ensured that the current node should be here and noted its layer we
            // can set up the next step of the search
            let predecessors = self.preds.get(&curr).unwrap();
        ...
        for sensor_node in &self.sensors {
            rev_layers.insert(*sensor_node, layer + 1);
        }
        ...
        for node in rev_layers.keys() {
            ...
            node_layers.insert(*node, layer + 1 - rev_layers[node]);
        }

        Ok(node_layers)
    }

    fn resolve_fully_connected_pair(&self, current_node: &usize, check_node: &usize) -> LayerAdjustment {
...
mod tests {
    ...
    fn singly_recurrent_graph() -> Graph {
        Graph::from_edge_list(vec![[0, 2], [1, 3], [3, 2], [3, 4], [2, 5], [4, 3], [4, 5]], vec![0, 1], vec![5])
    }
    ...
    fn test_path_length(
        ...
        assert_eq!(graph.get_path_length(&source, &target, backward_flag).unwrap(), expected)
    }
    ...
    fn test_get_layer_map(three_cycle_graph: Graph) {
        ...
        assert_eq!(known, three_cycle_graph.get_layer_map().unwrap())
    }

    #[rstest]
    fn test_get_layer_map_alt(singly_recurrent_graph: Graph) {
        ...
        assert_eq!(known, singly_recurrent_graph.get_layer_map().unwrap())
    }
    ...
    fn test_recurrent_pseudosort(#[case] graph: Graph, #[case] knowns: Vec<Result<Vec<usize>, ConnectivityError>>) {
